...
async function graphqlRequest(query, variables = {}) {
  ...
  if (!response.ok || json.errors) {
    const errMsg = json?.errors?.map((e) => e.message).join('; ') || response.statusText || 'GraphQL error';
...
async function sendNotification(config, index, total) {
  ...
  try {
    ...
    console.log(
      `✅ ${index + 1}/${total} sent:${attachmentInfo}${actionInfo} - ${config.title} (ID: ${message?.id || 'N/A'})`,
    );
